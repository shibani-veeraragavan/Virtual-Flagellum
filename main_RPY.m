function [Filament, t] = main_RPY(a,ds,Ns,S,A,k_b,phi,mu,KB,KT,wall,dtfac,num_settling_times,outfilename,save_step,concheck_tol) %#codegen
...
for nt = 1:TOTAL_STEPS  
    ...
    Filament = ActiveST(Filament,omega_b, k_b, phi, A, t);
    ...
    if(save_now == save_step && print_results)
        if nt == 1 
          ...
          fprintf('%.4e, %.6f, %.0f, %.4f, %.0f, %.0f, %.0f, %.2f, %.2f, %.2f, %.4f, %.4f, %.4f, %.0f, %.0f, %.0f, %.0f, %.2e \n',dt, a, Ns, L, A(1), A(2), A(3), S(1), S(2), S(3), k_b(1), k_b(2), k_b(3), mu, KB, KT, dtfac, concheck_tol);
        end
