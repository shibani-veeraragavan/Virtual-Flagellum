...
properties
    ...
    StrainTwist; % The strain-twist vector encoding the intrinsic (preferred) curvature and twist of the filament.
    mA; % Active moment vector.
...
methods
    ...
    function InitialSetup(obj,FirstSegPos,StrainTwist,B,Sp,omega_b,...
...
%         obj.Q = file(:,1:8);
        ...
        obj.KB = obj.Length^4/(Sp^4)*mu*omega_b(1);
% %         obj.KB = weight_per_unit_length*(obj.Length)^3 / B;
    ...
    function ActiveMoment(obj, omega_b, k_b, m_0, nt)
    ...
    %               active moment wave vector, and place into Filament.mA 
        ...
        q = obj.Q;
        mA_wave = zeros(3,N);
        s = linspace(0,1,N); % Arclength
        for i=1:N
            if m_0(4) == 0
                mA_wave(1,i) = m_0(1)*cos(k_b(1)*s(i) - omega_b(1)*nt);
                mA_wave(2,i) = m_0(2)*cos(k_b(2)*s(i) - omega_b(2)*nt);
                mA_wave(3,i) = m_0(3)*cos(k_b(3)*s(i) - omega_b(3)*nt);
                mA_wave(:,i) = QuaternionRotation(q(i,1:4),mA_wave(:,i));
            elseif m_0(4) == 1
                mA_wave(1,i) = m_0(1)*s(i)*cos(k_b(1)*s(i) - omega_b(1)*nt);
                mA_wave(2,i) = m_0(2)*s(i)*cos(k_b(2)*s(i) - omega_b(2)*nt);
                mA_wave(3,i) = m_0(3)*s(i)*cos(k_b(3)*s(i) - omega_b(3)*nt);
                mA_wave(:,i) = QuaternionRotation(q(i,1:4),mA_wave(:,i));    
            end
        end
        obj.mA = dL*mA_wave*Kb/L^2;
    end

    function NewStrainTwist(obj, omega, k, phi, m_0, t)
        L = obj.Length;
        st = [0, 0, 0];
        st(1) = m_0(1)*cos(k(1)*L/2 - omega(1)*t + phi(1))/L;
        st(2) = m_0(2)*cos(k(2)*L/2 - omega(2)*t + phi(2))/L;
        st(3) = m_0(3)*cos(k(3)*L/2 - omega(3)*t + phi(3))/L;
        obj.StrainTwist = st;
    end
    ...
    %                                      in Filament.F
        ...
        % Active moments
        ForcesAndTorques(4:6,:) = ForcesAndTorques(4:6,:) + ActiveM; %
